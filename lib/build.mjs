...
export async function build(repoRoot, { silent = false, quiet = false, verbose = false } = {}) {
  ...
  if (!silent) {
    if (quiet) console.log(`🗺️  Cartograph: map updated (${graph.stats.files} files)`);
...
function printSummary(graph, config, outDir, repoRoot) {
  ...
  console.log(`   → ${relative(repoRoot, outDir) || ".cartograph"}/ (ARCHITECTURE.md, graph.json)`);
}
...
function warnWatchDrift(repoRoot, config) {
  ...
  if (touchesWatch && !touchesMemory) {
    console.log(`\n🔖 Cartograph: you're changing code marked as "critical" (config.watch),`);
    console.log(`   but no memory/ note is staged. Consider recording a decision in .cartograph/memory/.`);
  }
}
