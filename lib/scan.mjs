...
// Match eines Nutzer-ignore-Patterns gegen einen repo-relativen Pfad — bewusst an Segmentgrenzen,
// damit "test" NICHT "src/contest/x.ts" rausfiltert. Pfad-Fragmente (mit "/") und Dateinamen-
// Fragmente (mit ".") matchen weiterhin als gebundenes Teilstück.
export function matchesIgnore(rel, pattern) {
  const pat = String(pattern || "").replace(/\/+$/, "");
  if (!pat) return false;
  const segs = rel.split("/");
  if (pat.includes("/")) {
    // Pfad-Fragment: nur an Segmentgrenzen
    return rel === pat || rel.startsWith(pat + "/") || rel.includes("/" + pat + "/") || rel.endsWith("/" + pat);
  }
  // Reiner Name: exaktes Segment
  if (segs.includes(pat)) return true;
  // Dateinamen-Fragment mit Punkt (z.B. ".test.", ".min.js") weiterhin als Teilstring im Basenamen
  return pat.includes(".") && segs[segs.length - 1].includes(pat);
}
...
function isIgnored(rel, patterns) {
  ...
  return patterns.some((p) => matchesIgnore(rel, p));
}
