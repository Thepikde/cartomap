...
import { reverseDeps, dependentsOf } from "./affected.mjs";
...
export function buildGraph(files, { config, ctx }) {
  ...
  // Hubs + verwaiste Module (eingehende Imports) + Blast-Radius (transitive Dependents)
  const inDegree = {};
  ...
  const hubs = Object.entries(inDegree)
    ...
    .slice(0, 12)
    .map(([file, n]) => ({ file, inDegree: n, affected: dependentsOf(rev, file).length }));
