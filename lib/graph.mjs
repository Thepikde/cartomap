...
export function buildGraph(files, { config, ctx }) {
  ...
  for (const [rel, info] of entries) {
    if (info.kind === "page" && info.route != null) pages.push({ url: info.route, file: rel });
    else if (info.kind === "api" && info.route != null) apiRoutes.push({ url: info.route, methods: info.methods || [], file: rel });
  }
  pages.sort((a, b) => a.url.localeCompare(b.url));
  apiRoutes.sort((a, b) => a.url.localeCompare(b.url));
