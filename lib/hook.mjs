...
//   - Sonst                              -> eigener .cartomap/hooks + core.hooksPath (team-tauglich)
...
const CHAIN_MARKER = "# >>> cartomap >>>";
const CHAIN_BLOCK = `${CHAIN_MARKER}
# Keep the Cartomap map up to date on every commit (added by \`cartomap install-hook\`).
if command -v cartomap >/dev/null 2>&1; then
  cartomap build --quiet 2>/dev/null && git add "$(git rev-parse --show-toplevel)/.cartomap/ARCHITECTURE.md" 2>/dev/null || true
fi
# <<< cartomap <<<`;
...
function detectExistingHooks(repoRoot) {
  ...
  if (hooksPath && hooksPath !== ".cartomap/hooks") {
...
export function installHook(repoRoot) {
  ...
  const hooksDir = join(repoRoot, ".cartomap", "hooks");
  mkdirSync(hooksDir, { recursive: true });
  const dest = join(hooksDir, "pre-commit");
  copyFileSync(HOOK_TEMPLATE, dest);
  chmodSync(dest, 0o755);
  git(repoRoot, ["config", "core.hooksPath", ".cartomap/hooks"]);
