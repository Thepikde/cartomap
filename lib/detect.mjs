...
// Das ist die Grundlage dafür, dass Cartograph sich an JEDES Projekt anpasst —
...
// Pfad-Aliase aus tsconfig.json paths ableiten:  "@/*": ["./src/*"]  ->  { "@/": "src/" }
export function readTsconfigAliases(repoRoot) {
  const tsconfig = readJsonLoose(join(repoRoot, "tsconfig.json"));
  const aliases = {};
